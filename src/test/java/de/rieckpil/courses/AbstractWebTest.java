...
public abstract class AbstractWebTest {
  ...
  private static final DockerComposeContainer<?> environment = populateEnvironment();

  private static DockerComposeContainer<?> populateEnvironment() {
    try (DockerComposeContainer<?> dcc = new DockerComposeContainer<>(new File("docker-compose.yml"))) {
      return dcc.withExposedService("database_1", 5432, Wait.forListeningPort())
